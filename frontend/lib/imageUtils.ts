 ...
 * - Продакшн: http://141.105.71.70:8080/api/files/{filename}
 * 
 * Преимущества прокси:
...
/**
 * Получает базовый URL для Spring Boot API
 */
function getApiBaseUrl(): string {
  return process.env.NEXT_PUBLIC_API_BASE_URL || 
    (process.env.NODE_ENV === 'production' ? 'http://141.105.71.70:8080' : 'http://localhost:8080');
}
...
export function getImageUrl(imageUrl?: string): string {
  ...
  return `${getApiBaseUrl()}/api/files/${imageUrl}`;
}
